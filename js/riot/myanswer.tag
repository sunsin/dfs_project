<myanswer>
	<div class="forword" onclick="{ gohistory }">
	    <div class="back"><span>我的回答</span></div>
	</div>

	<audio src="" id="audio"></audio>
	<answer-item each="{ myanswer }" class="answer-item"></answer-item>
	<script>
		var self = this;
...
<answer-item>
	...
	<div class="answeruser" onclick="{  }" if="{ isanswer }">
		<div>
			<span>答</span>
	...
	<div class="state">
		<div class="isanswer waitToAnswer" if={ !isanswer && state != "isexpired"}>待回答</div>
		<div class="isanswer" if={ state = isexpired &&  !isanswer}>已过期</div>
	</div>
	<div class="chooseWay">
		<div class="audio" onclick="{ toAudioAnswer }"><span>语音回答</span></div>
		<div class="pictxt" onclick="{ toPicsTxtAnswer }"><span>图文回答</span></div>
	</div>
	...
	<script>
		...
		toAudioAnswer(){
			var url = "answeritaudio.html?id="+self._id;
			window.location.href = url;
		}
		toPicsTxtAnswer(){
			var url = "answerittext.html?id="+self._id;
			window.location.href = url;
		}

		$(function(){
			$(".waitToAnswer").click(function(){
				$(this).parent().parent().find(".chooseWay").show();
			})
		})
