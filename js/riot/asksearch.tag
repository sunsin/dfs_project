<answer class="answer">
	...
	<div class="forword" onclick="{ gohistory }">
	    <div class="back"><span>搜索问答</span></div>
	</div>
	<div class="videoSearch">
		<div style="width: 80%;padding: 5px;">
			<input type="search" name="" placeholder="搜索问题或医生" onkeyup="{ editor }">
		</div>
	...
	<script>
		...
		// self.query = decodeURI(location.href.split("query=")[1].split("&")[0]);
        ...
        search(){
			...
			RiotControl.trigger("getAskAnswerList",infos);
	        RiotControl.on("askAnswerListChanged",function(data){
