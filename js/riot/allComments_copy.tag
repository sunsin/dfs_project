    ...
    //获取评论列表
        ...
        $(function(){
            ...
            $(".discuss").find("button").click(function(){
                let holder = $("input").attr("placeholder").split("：")[0];
                //let replyUser = $("input").attr("placeholder").split("：")[1];
                if(holder != "回复"){
                    ...
                    // 评论帖子
                    let replyPostUrl = "http://api.daifushuo.com:80/api/ocomments?access_token="+accessToken;
                    ...
                    $.get(userUrl,discussVal,function(data){
                        let userMessage ;
                        userMessage = data;
                        let discussVal = $(".discuss").find("input").val();
                        if(discussVal != null){
    ...
    <script>
        ...
        reply(){
            if(this.text){
                ...
                let replyCommentUrl = "http://api.daifushuo.com:80/api/ocomments/dfs_reply_comment?access_token="+dfs_cookie.id;
                let replyAll = {
                    "reply": reply,
                    "url": replyCommentUrl
                }
                ...
                let userUrl = "http://api.daifushuo.com:80/api/ousers/"+ dfs_cookie.userId +"?access_token="+dfs_cookie.id;
                $.get(userUrl,function(data){
                    let userMessage ;
                    userMessage = data;
                    let discussBox = "<all-item><div class='commentItem'><img src='"+ userMessage.avatar +"' class='commentUserImg'><div><div class='line1'><div class='nickname'>"+ userMessage.nickname +"</div><div class='create'>"+ "刚刚" +"</div></div><div class='line2'><span class='content'>" + replyText + "</span></div></div></div></all-item>";
                        $(".commentList").prepend(discussBox);
                    $("input").val("").focus();
                });
            }
        ...
        delete(){
            let deleteUrl = "http://api.daifushuo.com:80/api/ocomments/" + this._id + "?access_token=" + dfs_cookie.id;
            $.ajax({
                    url: deleteUrl,
                    type: 'DELETE',
